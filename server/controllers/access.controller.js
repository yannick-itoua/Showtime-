...
const login = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const token = jwt.sign({id: user._id,name: user.name}, jwtConfig.SECRET_TOKEN, {expiresIn : '3h'} );
      console.log(`login : ${token}`);
      res.cookie('token', token,  { maxAge : 3*60*60*1000, httpOnly: true, sameSite : 'strict' })  // secure : true (avec https)
      res.status(200).json({ message : 'utilisateur connecté',admin: user.admin });
    }
    else { // unknown login
